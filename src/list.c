...
int
sa_list_prepend(sa_list *lst, sa_list_val value) {
...
int
sa_list_append(sa_list *lst, sa_list_val value) {
  assert(lst != NULL && value != NULL);
  sa_list_node *node = sa_list_node_new();
  if (node == NULL) {
    return -1;
...
sa_list_node *
sa_list_find(sa_list *lst, sa_list_val val) {
  assert(lst != NULL);
  assert(lst->equals_fn != NULL);
  
  sa_list_node *lst_node = lst->head;
  while (lst_node != NULL) {
    printf("comparing '%s' with '%s'\n", lst_node->value, val);
    if (lst->equals_fn(lst_node->value, val) == true) {
      printf("returning this value\n");
      return lst_node;
    }
    lst_node = lst_node->next;
  }
...
sa_list_remove(sa_list *lst, sa_list_node *node) {
  ...
  sa_list_val node_value = node->value;
